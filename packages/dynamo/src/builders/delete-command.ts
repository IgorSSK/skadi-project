...
import { DeleteCommand as AwsDeleteCommand, DynamoDBDocumentClient } from "@aws-sdk/lib-dynamodb";
 ...
 */
export class DeleteCommand<TSchema extends TableSchema<Record<string, unknown>>> {
	constructor(
	...
	async exec(): Promise<[null, Error | null]> {
		try {
			...
			return [null, new DynamoCommandError("Erro ao executar DeleteCommand", error)];
		}
	}
}
