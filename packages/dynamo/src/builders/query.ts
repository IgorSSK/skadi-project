...
class QueryKeyBuilder<
  TSchema extends z.ZodObject<EntitySchemaDefinition>,
> extends BaseBuilder<QueryResult<z.infer<TSchema>>> {
  ...
  filter(
    field: string,
    operator: '=' | '<' | '<=' | '>' | '>=' | 'contains' | 'begins_with' | 'IN',
    value: unknown
  ) {
  ...
  async exec(): Promise<DynamoResult<QueryResult<z.infer<TSchema>>>> {
    ...
    const [output, opErr] = await this.send<QueryCommandOutput>(
    ...
    try {
      ...
      return [
        {
          items,
          cursor: output?.LastEvaluatedKey as
  ...
  stream() {
    return {
       ...
       */
      process: async (
        processor: (item: z.infer<TSchema>) => Promise<void>,
        ...
        do {
          ...
          cursor =
            typeof result.cursor === 'string' ? result.cursor : undefined;
        } while (cursor);
      },
    };
  }
