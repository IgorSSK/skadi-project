  ...
  async exec(): Promise<[null, Error | null]> {
    try {
      ...
      if (updateKeys.length === 0) {
        return [
          null,
          new DynamoCommandError('Nenhum campo para atualizar', null),
        ];
      }
      ...
      const ExpressionAttributeValues = Object.fromEntries(
        updateKeys.map(k => [
          `:${k}`,
          (this.updates as Record<string, unknown>)[k],
        ])
      );
      ...
      return [
        null,
        new DynamoCommandError('Erro ao executar UpdateCommand', error),
      ];
    }
  }
}
