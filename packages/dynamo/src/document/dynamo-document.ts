...
import type { ExtractPrimaryKeys, InferItemType } from "../types/schema-utils.js";
...
export class DynamoDocument<T> {
	 ...
	 */
	async put(item: InferItemType<TableSchema<T>["definition"]>): Promise<[null, Error | null]> {
	...
	): Promise<[InferItemType<TableSchema<T>["definition"]> | null, Error | null]> {
	 ...
	 */
	async delete(keys: Partial<ExtractPrimaryKeys<TableSchema<T>["definition"]>>): Promise<[null, Error | null]> {
