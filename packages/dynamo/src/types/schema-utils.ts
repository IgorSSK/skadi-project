...
export type InferItemType<TDef extends TableSchemaDefinition<unknown>> = {
  [K in keyof TDef]: TDef[K] extends Attribute<infer T>
    ? T
    : TDef[K] extends TemplateKey<infer T>
      ? T
      : never;
};
...
export type ExtractPrimaryKeys<TDef extends TableSchemaDefinition<unknown>> = {
  [K in keyof TDef as TDef[K] extends TemplateKey<unknown>
    ? K
    : never]: TDef[K] extends TemplateKey<infer T> ? T : never;
};
...
export type ExtractAttributes<TDef extends TableSchemaDefinition<unknown>> = {
  [K in keyof TDef as TDef[K] extends Attribute<unknown>
    ? K
    : never]: TDef[K] extends Attribute<infer T> ? T : never;
};
