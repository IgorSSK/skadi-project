 ...
 */
export interface DynamoClientConfig extends Omit<DynamoDBClientConfig, "region"> {
...
export class DynamoClientFactory implements IClientFactory {
	...
	private readonly documentClientCache = new Map<string, DynamoDBDocumentClient>();

	/**
	 * Creates a DynamoDB document client with enhanced configuration
	 */
	public createDocumentClient(region: string, config?: Partial<DynamoClientConfig>): DynamoDBDocumentClient {
	 ...
	 */
	public createClient(region: string, config?: Partial<DynamoClientConfig>): DynamoDBClient {
		const cacheKey = this.createCacheKey(region, config);
	 ...
	 */
	private createCacheKey(region: string, config?: Partial<DynamoClientConfig>): string {
		if (!config) {
			return region;
		}
 ...
 */
export function createDocumentClient(region: string, config?: Partial<DynamoClientConfig>): DynamoDBDocumentClient {
 ...
 */
export function createClient(region: string, config?: Partial<DynamoClientConfig>): DynamoDBClient {
