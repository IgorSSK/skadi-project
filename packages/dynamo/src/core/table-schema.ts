...
import type { SchemaDefinition, TableConfiguration } from "../types/index.js";
...
export class TableSchema<T extends Record<string, unknown>> {
	 ...
	 */
	public static define<T extends Record<string, unknown>>(name: string, schema: SchemaDefinition<T>): TableSchema<T> {
	 ...
	 */
	public withBillingMode(billingMode: "PAY_PER_REQUEST" | "PROVISIONED"): TableSchema<T> {
		return new TableSchema({
			...this.config,
			billingMode,
		});
	 ...
	 */
	public withTimeToLive(attributeName: keyof T, enabled = true): TableSchema<T> {
		return new TableSchema({
			...this.config,
	...
	private validateSchema(): void {
		...
		if (primaryKeys.length === 0) {
			throw new DynamoConfigurationError("Schema must have at least one primary key (HASH)");
		}
		...
		if (hashKeys.length !== 1) {
			throw new DynamoConfigurationError("Schema must have exactly one partition key (HASH)");
		}

		if (rangeKeys.length > 1) {
			throw new DynamoConfigurationError("Schema can have at most one sort key (RANGE)");
		}
		...
		if (this.config.timeToLive) {
			...
			if (!(ttlAttr in schema)) {
				throw new DynamoConfigurationError(`TTL attribute '${String(ttlAttr)}' is not defined in schema`);
			}
		}
	}
	...
	private getPrimaryKeyAttributes(): PrimaryKeyAttributeImpl<unknown>[] {
		return Object.values(this.config.schema).filter(
			(attr): attr is PrimaryKeyAttributeImpl<unknown> => "isPrimary" in attr && attr.isPrimary === true,
		);
	}
	...
	public validateItem(item: unknown): item is T {
		...
		try {
			for (const [fieldName, attribute] of Object.entries(schema)) {
				...
				if ("validateValue" in attribute && typeof attribute.validateValue === "function") {
	 ...
	 */
	private getFieldNameForAttribute(targetAttr: PrimaryKeyAttributeImpl<unknown>): string | undefined {
		const { schema } = this.config;
...
export const TableSchemaLegacy = {
	 ...
	 */
	create: <T extends Record<string, unknown>>(name: string, definition: SchemaDefinition<T>): TableSchema<T> => {
	 ...
	 */
	pk: <T = string>(attrName = "pk", template?: string): PrimaryKeyAttributeImpl<T> => {
	 ...
	 */
	sk: <T = string>(attrName = "sk", template?: string): PrimaryKeyAttributeImpl<T> => {
