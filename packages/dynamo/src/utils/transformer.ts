...
import { camelCase as lodashCamelCase, snakeCase as lodashSnakeCase, upperFirst } from "lodash-es";
...
const pascalCase: CaseFunction = (str: string) => upperFirst(lodashCamelCase(str));
...
const transformObjectKeys = (obj: unknown, transformFn: CaseFunction): unknown => {
...
const marshallWithDateConversion = (value: unknown): unknown => {
	...
	if (isObject(value)) {
		return Object.fromEntries(Object.entries(value).map(([key, val]) => [key, marshallWithDateConversion(val)]));
	}
	return value;
};
...
const unmarshallWithDateConversion = (value: unknown): unknown => {
	if (isDate(value)) {
		const parsed = typeof value === "string" ? parseDateString(value) : (value as Date);
	...
	if (isObject(value)) {
		return Object.fromEntries(Object.entries(value).map(([key, val]) => [key, unmarshallWithDateConversion(val)]));
	}
	return value;
};
...
function reverseTemplate(
	...
	while (match) {
		...
		pattern += template.slice(lastIdx, match.index).replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
	...
	const coerced = unmarshallWithDateConversion(rawObj) as Record<string, unknown>;
...
function rehydrateKeysWithSchema(
	...
	for (const [field, fieldSchema] of Object.entries(shape)) {
		const meta: KeyTemplateMeta | undefined = (fieldSchema as z.ZodTypeAny & { _skadiKeyMeta?: KeyTemplateMeta })
			._skadiKeyMeta;
		if (!meta) continue;
...
export const serialize = (data: UnknownObject, caseStyle?: CaseTransformer): UnknownObject => {
...
export function deserialize(data: UnknownObject, schema?: z.ZodObject<Record<string, z.ZodTypeAny>>): UnknownObject {
	// For deserialization, we always convert back to camelCase first
	const transformFn = caseFunctions.camelCase as CaseFunction;
	const transformedData = transformObjectKeys(data, transformFn) as UnknownObject;
